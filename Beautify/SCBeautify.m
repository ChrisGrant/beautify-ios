...
-(void)activateWithStyle:(NSString*)styleName {
    ...
    SCTheme* theme = [SCTheme fromFile:styleName];
    if(theme) {
        [[SCThemeManager instance] applyTheme:theme];
    }
    [self activateInternal];
}
...
-(void)swizzleAll {
    // Creates renderers for the class specified here [SCBeautify instance].renderers
    for (NSString *classToSwizzle in [SCThemeManager instance].renderers.keyEnumerator) {
    ...
    // Creates a instance of SCDelegatemultiplexer to act as a proxy delegate for UITextField
